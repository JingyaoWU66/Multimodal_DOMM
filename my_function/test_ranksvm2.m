function [w,rank] = test_ranksvm2(XXall,YYall)
  %load ohsumed
  
 % clear all;close all;
 % load('iemocap_training_set.mat');

  C = 1e-4; % Constant penalizing the training errors
  for i=1:1
      
   % ex{1} = Yall(i).Y{1};
    %A = generate_constraints(ex);
    A = generate_constraints(YYall(i).Y); %用来计算pair的
    tic;
    
   % xfeature = Xall(i).X(1:138,:);
    %w = ranksvm(xfeature, A, C*ones(size(A,1),1));
    w = ranksvm(XXall(i).X, A, C*ones(size(A,1),1));
    t = toc;
    rank{i} = XXall(i).Xt*w;%Xall.Xt： test features
   % ndcg(i) = compute_ndcg(XXall(i).Xt*w,YYall(i).Yt,10); %Yall.Yt: test rank
...
function A = generate_constraints(Y)
  ...
  A = sparse(I(1:2*nt),J(1:2*nt),V(1:2*nt));    %I:row index, J: col index, V: the value
end
